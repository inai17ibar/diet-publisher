...
- **100点満点の採点**: 記録40点（3食そろった日数/7）+ 目標カロリー35点（目標内の日数/記録日数）+ タンパク質25点（135g達成の日数/記録日数）。S/A/B/C/Dのグレード付き
  - 目標カロリー未設定の週は、その項目を除いて100点満点に換算し直す
  - 配点は `app/services/weekly_review.py` の `POINTS_*` 定数で調整できる
- **来週の改善ポイント**: gpt-4oが週の実データと採点結果から生成（点数や差分はPython側で決定的に計算し、AIには講評だけ書かせる）

投稿タイミングは日次と同じく状態駆動で、`.github/workflows/story-publish.yml` の1日6回のcronに相乗りしている。

1. 日曜のJST21時以降なら、終わろうとしている**今週**を投稿
2. それ以外は**先週**が未投稿なら投稿（日曜夜の取りこぼし救済）

記録が3日未満の週は振り返りとして成立しないため投稿しない（`too_few_records`。`MIN_RECORDED_DAYS` で調整可）。`/story/weekly-image` はこの制限を受けないので、少ない週でも手元で確認できる。
