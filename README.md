...
- Instagramストーリーへの自動投稿（公式Graph API・1日3回のcronで状態駆動）
...
- `.github/workflows/story-publish.yml` が **1日3回（8/13/22時 JST）** `/story/publish` を叩き、サーバー側が記録の状態を見て投稿すべき日を選ぶ: ①21時以降で今日に記録があれば今日を投稿 ②今日が未投稿でも、昨日に記録があり未投稿なら昨日を投稿（翌朝入力パターンの救済）。冪等なので何度呼んでも二重投稿しない。GitHub Secrets に `DIET_PUBLISHER_API_KEY`（= `SECRET_KEY`）が必要。認証情報が未設定の間は `not_configured` を返すだけでcronは失敗しない
...
投稿タイミングは日次と同じく状態駆動で、`.github/workflows/story-publish.yml` の1日3回のcronに相乗りしている。今週・先週の順に見て、締まっていて未投稿の週があればそれを投稿する。週の途中は日曜がまだ空なので、この門番がフライング投稿も防いでいる。
